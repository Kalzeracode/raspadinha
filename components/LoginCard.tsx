...
import { login, getMe } from './apiService';
...
interface LoginCardProps {
    ...
    onLogin: (data: { token: string, user: { name: string, credits: number } }) => void;
}

const LoginCard: React.FC<LoginCardProps> = ({ onNavigateToRegister, onLogin }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // Combina o token e os dados do usuário para o estado global do app
            onLogin({
                token,
                user: {
                    // O backend precisa retornar `fullName` e `credits` na rota /me
                    name: userData.fullName || userData.email, // Fallback para email se nome não vier
                    credits: userData.credits || 0, // Fallback para 0 se créditos não vierem
                },
            });
        ...
        <div className="w-full max-w-md mx-auto bg-slate-800/50 backdrop-blur-lg rounded-2xl shadow-2xl shadow-black/30 overflow-hidden border border-slate-700">
            ...
            <div className="bg-slate-900/50 px-8 py-4 text-center text-sm">
                <p className="text-slate-400">
                    Ainda não tem conta?{' '}
                    <button onClick={onNavigateToRegister} className="font-medium text-brand-yellow hover:text-brand-pink underline transition-colors duration-300 bg-transparent border-none cursor-pointer p-0">
                        Cadastre-se
                    </button>
                </p>
            </div>
